...
const TimeBoundQRCodes = () => {
  ...
  const getQrcodeList = useCallback(async () => {
    ...
    setLoading(true);
    await getTimeBoundQRCodes(currentOrgId)
      .then((res) => {
        if (!res.error) {
          setQrcodeData(res.data.filter((val) => val.type == 'timebound'));
        }
      })
      .finally(() => {
        setLoading(false);
      });
  }, [currentOrgId, selectedCategories, categories]);
      ...
      <Container sx={{ marginTop: { sm: -3, xs: -1 } }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={4}>
          ...
          <Button
            ...
            onClick={() => navigate('/app/timeQRCodes/create')}
            hidden
          >
            Create
          </Button>
...
export default TimeBoundQRCodes;
